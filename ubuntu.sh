...
install1 () {
    directory=ubuntu-fs
    UBUNTU_VERSION='22.04'  # Changed to Jammy Jellyfish
    if [ -d "$directory" ]; then
        first=1
    ...
    if [ "$first" != 1 ]; then
        ...
        # Updated nameserver to Google's DNS
    ...
    # Script content for starting Ubuntu
    cat > $bin <<• EOM
#!/bin/bash
cd \$(dirname \$0)
...
unset LD_PRELOAD
command="proot"
# Uncomment this line if you are having FATAL: kernel too old message.
# command+=" -k 4.14.81"
